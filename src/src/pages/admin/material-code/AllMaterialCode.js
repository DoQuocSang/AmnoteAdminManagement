...
import { getAllMaterialCode } from "services/MaterialCodeRepository";

export default () => {
  document.title = "Quản lý mã nhóm vật tư";
  ...
  const [materialCodesToDelete, setMaterialCodesToDelete] = useState([]);

  // Tính thông số phân trang
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = materialCodeList.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const childToParent = (isDelete) => {
    if (isDelete === true && materialCodesToDelete.length > 0) {
      const filteredMaterialCodes = materialCodeList.filter(
        (materialCode) => !checkedItems[materialCode.PRODUCTKIND_CD]
      );
      setMaterialCodeList(filteredMaterialCodes);
      const resetCheckedItems = {};
      filteredMaterialCodes.forEach((materialCode) => {
        resetCheckedItems[materialCode.PRODUCTKIND_CD] = false;
      });
      setCheckedItems(resetCheckedItems);
    }
    // console.log(materialCodeList.length);
  };

  const handleRowClick = (data) => {
    // dispatch(setData(data));
    sessionStorage.setItem('editItem', JSON.stringify(data));
    navigate(`/dashboard/update-material-code/${data.PRODUCTKIND_CD}`);
  };
  ...
  useEffect(() => {
    ...
    // Xử lý logic
    getAllMaterialCode().then((data) => {
      if (data) {
        setMaterialCodeList(data.result.slice().reverse());

        // Checkbox
        const initialCheckedItems = {};
        data.result.slice().reverse().forEach((materialCode) => {
          initialCheckedItems[materialCode.PRODUCTKIND_CD] = false;
        });
  ...
  const handleCheckboxChange = (PRODUCTKIND_CD) => {
    setCheckedItems((prevCheckedItems) => ({
      ...prevCheckedItems,
      [PRODUCTKIND_CD]: !prevCheckedItems[PRODUCTKIND_CD],
    }));
  };

  const handleDelete = () => {
    const materialCodes = Object.keys(checkedItems).filter(
      (key) => checkedItems[key]
    );
    setMaterialCodesToDelete(materialCodes);
  };
              ...
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-base font-normal text-gray-500">
                    Các mã nhóm vật tư hiện có trong database
                  </span>
                </div>
                ...
                <div className="flex-shrink-0">
                  ...
                  >
                    <Link to="/dashboard/add-material-code">
                            ...
                            <tbody className="bg-white">
                              {currentItems.map((item, index) => (
                                ...
                                >
                                      ...
                                      <div class="inline-flex items-center">
                                        ...
                                        >
                                          <input
                                            type="checkbox"
                                            checked={
                                              checkedItems[item.PRODUCTKIND_CD]
                                            }
                                            onChange={() =>
                                  ...
                                  <td className="p-4 whitespace-nowrap text-sm">
                                    {item.PRODUCTKIND_NM_ENG}
                                  </td>
                                  <td className="p-4 whitespace-nowrap text-sm">
                                    {item.PRODUCTKIND_NM_KOR}
                                  </td>
                                  <td className="p-4 whitespace-nowrap text-sm">
                                    {item.REMARK}
                                  </td>
                    ...
                    {materialCodeList.length === 0 ? (
                      ...
                      <>
                        <Pagination
                          ...
                          totalItems={materialCodeList.length}
                          handlePageChange={paginate}
                          itemsPerPage={itemsPerPage}
                        />
                        <DeleteModal
                          isDelete={childToParent}
                          deleteItems={materialCodesToDelete}
                          type="material-code"
                        />
                      </>
                    )}
